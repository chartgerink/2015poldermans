...
result_almost_i_uncertain <- matrix(nrow = iterations, ncol = length(xs))
...
for (x in xs){
  for (iter in 1:iterations){
    ...
    result_almost_i_uncertain[iter, j] <- decrease_p > non_decrease_p 
...
result_almost_iv_uncertain <- matrix(nrow = iterations, ncol = length(xs))
...
for (x in xs){
  for (iter in 1:iterations){
    ...
    result_almost_iv_uncertain[iter, j] <- decrease_p > non_decrease_p 
